...
async def test_condition_expression_supports_comparison_membership_and_boolean_logic():
    state = {
        "context": {
            "score": 91,
            "level": "vip",
    ...
    assert evaluate_when("{{context.score}} >= 90", state)
    assert evaluate_when("{{context.score}} < 100 and {{context.level}} in ['vip', 'svip']", state)
    assert evaluate_when("'prod' in {{context.tags}}", state)
    assert evaluate_when("{{context.level}} not in ['normal', 'guest']", state)
    assert evaluate_when("{{context.phrase}} == 'rock and roll' or {{context.score}} < 10", state)
    assert not evaluate_when("{{context.score}} <= 80 or {{context.level}} == 'guest'", state)

async def test_human_response_schema_validates_resume_payload():
    ...
    assert required.data["request"]["fields"][0]["id"] == "env"

    with pytest.raises(WorkflowError, match="missing required field: version"):
        [
            event
...
async def test_tool_input_schema_rejects_invalid_args():
    agents = AgentRegistry()
    tools = ToolRegistry()
    calls = []
    ...
    assert "tool deploy input.env" in events[-1].data["error"]

async def test_tool_output_schema_rejects_invalid_output():
    agents = AgentRegistry()
    tools = ToolRegistry()
    ...
    assert events[-1].type == "run.failed"
    assert "tool bad output.ok must be boolean" in events[-1].data["error"]

async def test_tool_input_schema_rejects_string_length_and_number_range():
    agents = AgentRegistry()
    tools = ToolRegistry()
    calls = []
    ...
    assert calls == []
    assert events[-1].type == "run.failed"
    assert "tool deploy input.version length must be >= 5" in events[-1].data["error"]

async def test_schema_additional_properties_false_rejects_extra_fields():
    with pytest.raises(WorkflowError, match="payload has unsupported field: extra"):
        validate_schema_value(
...
async def test_schema_additional_properties_schema_validates_extra_fields():
    validate_schema_value(
        {"known": "ok", "dynamic": 2},
        {
            "type": "object",
    ...
    with pytest.raises(WorkflowError, match="payload.dynamic must be >= 1"):
        validate_schema_value(
            {"known": "ok", "dynamic": 0},
            {
                "type": "object",
...
async def test_schema_rejects_unknown_keywords_at_runtime():
    with pytest.raises(WorkflowError, match="unsupported schema keyword: oneOf"):
        validate_schema_value(
            {"name": "deploy"},
            {
                "type": "object",
...
async def test_workflow_config_rejects_invalid_schema_constraints():
    with pytest.raises(WorkflowConfigError, match="minLength must be <= maxLength"):
...
async def test_workflow_config_rejects_unknown_schema_keywords():
    with pytest.raises(WorkflowConfigError, match="unsupported schema keyword: pattern"):
        WorkflowConfig.from_dict(
            {
...
async def test_agent_output_schema_rejects_invalid_output():
    agents = AgentRegistry()
    tools = ToolRegistry()
    ...
    assert events[-1].type == "run.failed"
    assert "agent planner output.requires_confirmation must be boolean" in events[-1].data["error"]

async def test_node_output_can_be_stored_as_artifact():
    agents = AgentRegistry()
    tools = ToolRegistry()
    ...
    assert ref["node_id"] == "large"
    assert value == {"text": "x" * 100}

async def test_resolve_artifacts_replaces_nested_artifact_refs():
    artifacts = InMemoryArtifactStore()
    ...
    assert resolved == {"items": [{"text": "hello"}], "keep": "plain"}

async def test_node_input_can_resolve_artifact_refs():
    agents = AgentRegistry()
    tools = ToolRegistry()
    artifacts = InMemoryArtifactStore()
    seen = []
    ...
    assert seen == [{"document": {"payload": {"text": "large result"}}}]
    assert consumer_finished.data["output"] == {"received": "large result"}

async def test_artifact_store_provider_can_be_registered():
