...
async def test_expired_human_node_uses_on_timeout_decision():
    ...
    assert final.data["output"] == {"decision": "timeout", "timed_out": True}

async def test_resume_expired_actions_applies_on_timeout_decision():
    agents = AgentRegistry()
    tools = ToolRegistry()
...
async def test_resume_expired_actions_marks_action_expired_without_on_timeout():
    agents = AgentRegistry()
    tools = ToolRegistry()
    ...
    assert events[0].type == "human.expired"
