...
async def test_human_node_resume_continues_same_run():
...
async def test_sse_message_events_keep_same_down_message_id_across_resume():
    agents = AgentRegistry()
    agents.register("planner", planner_agent)
    agents.register("executor", executor_agent)
    ...
    assert "FINISH" not in [payload["event"] for payload in first_payloads]
    ...
    assert second_payloads[-1]["event"] == "FINISH"
    assert all(payload["down_message_id"] == down_message_id for payload in second_payloads)

async def test_transform_and_condition_edges():
    agents = AgentRegistry()
    tools = ToolRegistry()
    ...
    assert final.data["output"]["summary"] == "level=vip"

async def test_condition_node_writes_decision_for_branching():
    agents = AgentRegistry()
    tools = ToolRegistry()
    ...
    assert condition.data["output"]["value"] == "vip"
    assert final.data["output"]["route"] == "vip"